...
class OrderProvider extends ChangeNotifier {
  ...
  final StreamController<List<PoliedroOrder>> _ordersStreamController =
      StreamController<List<PoliedroOrder>>.broadcast();
  ...
  List<PoliedroOrder> get pendingOrders => _orders
      .where((order) => order.status == OrderStatus.pending.value)
      .toList();

  List<PoliedroOrder> get inProgressOrders => _orders
      .where((order) => order.status == OrderStatus.inProgress.value)
      .toList();

  List<PoliedroOrder> get completedOrders => _orders
      .where((order) => order.status == OrderStatus.completed.value)
      .toList();

  List<PoliedroOrder> get archivedOrders => _orders
  ...
  Future<void> loadOrders() async {
    ...
    if (ordersJson != null) {
      ...
      _orders =
          decodedOrders.map((order) => PoliedroOrder.fromJson(order)).toList();
      notifyListeners();
    }
  }
  ...
  Stream<List<PoliedroOrder>> getOrdersStream() {
    print('Iniciando stream de pedidos para a tela da cozinha');

    // Carregar pedidos do Firestore e adicionar ao stream
    loadOrdersFromFirestore().then((_) {
      print('Pedidos carregados do Firestore: ${_orders.length} pedidos');
      print('Pedidos pendentes: ${pendingOrders.length} pedidos');
      _ordersStreamController.add(_orders);
    });
    ...
    _firestore.collection('orders').snapshots().listen((snapshot) {
      print(
          'Snapshot recebido do Firestore: ${snapshot.docs.length} documentos');
      ...
      print('Pedidos processados do snapshot: ${_orders.length} pedidos');
      print(
          'Pedidos pendentes após processamento: ${pendingOrders.length} pedidos');
